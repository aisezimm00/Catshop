const data = [
  {
    "shop": 31,
    "category": [ ],
    "title": "architectures",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 39,
    "currency": "Chilean Peso",
    "id": "3"
  },
  {
    "shop": 73,
    "category": [ ],
    "title": "West",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 4,
    "currency": "Swedish Krona",
    "id": "4"
  },
  {
    "shop": 30,
    "category": [ ],
    "title": "Coves",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 9,
    "currency": "North Korean Won",
    "id": "5"
  },
  {
    "shop": 25,
    "category": [ ],
    "title": "West",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 99,
    "currency": "Lempira",
    "id": "6"
  },
  {
    "shop": 42,
    "category": [ ],
    "title": "oh",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 2,
    "currency": "Chilean Peso",
    "id": "7"
  },
  {
    "shop": 73,
    "category": [ ],
    "title": "Southeast",
    "description": "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 68,
    "currency": "Algerian Dinar",
    "id": "8"
  },
  {
    "shop": 74,
    "category": [ ],
    "title": "gold",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 64,
    "currency": "Tenge",
    "id": "9"
  },
  {
    "shop": 42,
    "category": [ ],
    "title": "Jaguar",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 22,
    "currency": "Quetzal",
    "id": "10"
  },
  {
    "shop": 52,
    "category": [ ],
    "title": "whip",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 2,
    "currency": "Pound Sterling",
    "id": "11"
  },
  {
    "shop": 77,
    "category": [ ],
    "title": "Trail",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 5,
    "currency": "Iraqi Dinar",
    "id": "12"
  },
  {
    "shop": 88,
    "category": [ ],
    "title": "innovative",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 14,
    "currency": "Guarani",
    "id": "13"
  },
  {
    "shop": 37,
    "category": [ ],
    "title": "enthrall",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 28,
    "currency": "Liberian Dollar",
    "id": "14"
  },
  {
    "shop": 90,
    "category": [ ],
    "title": "Fresh",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 32,
    "currency": "Belize Dollar",
    "id": "15"
  },
  {
    "shop": 69,
    "category": [ ],
    "title": "silver",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 29,
    "currency": "Chilean Peso",
    "id": "16"
  },
  {
    "shop": 29,
    "category": [ ],
    "title": "PCI",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 80,
    "currency": "Kenyan Shilling",
    "id": "17"
  },
  {
    "shop": 14,
    "category": [ ],
    "title": "Paradigm",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 88,
    "currency": "Gourde",
    "id": "18"
  },
  {
    "shop": 81,
    "category": [ ],
    "title": "purple",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 74,
    "currency": "Surinam Dollar",
    "id": "19"
  },
  {
    "shop": 86,
    "category": [ ],
    "title": "Forward",
    "description": "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 11,
    "currency": "Kenyan Shilling",
    "id": "20"
  },
  {
    "shop": 48,
    "category": [ ],
    "title": "Product",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 84,
    "currency": "Kwacha",
    "id": "21"
  },
  {
    "shop": 42,
    "category": [ ],
    "title": "HEX",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 16,
    "currency": "Ethiopian Birr",
    "id": "22"
  },
  {
    "shop": 20,
    "category": [ ],
    "title": "VGA",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 45,
    "currency": "Philippine Peso",
    "id": "23"
  },
  {
    "shop": 29,
    "category": [ ],
    "title": "Funk",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 83,
    "currency": "New Israeli Sheqel",
    "id": "24"
  },
  {
    "shop": 25,
    "category": [ ],
    "title": "woman",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 75,
    "currency": "European Unit of Account 9(E.U.A.-9)",
    "id": "25"
  },
  {
    "shop": 13,
    "category": [ ],
    "title": "corrupti",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 14,
    "currency": "Trinidad and Tobago Dollar",
    "id": "26"
  },
  {
    "shop": 22,
    "category": [ ],
    "title": "Electric",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 75,
    "currency": "Sri Lanka Rupee",
    "id": "27"
  },
  {
    "shop": 27,
    "category": [ ],
    "title": "Kansas",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 79,
    "currency": "UAE Dirham",
    "id": "28"
  },
  {
    "shop": 81,
    "category": [ ],
    "title": "Latin",
    "description": "Andy shoes are designed to keeping in mind durability as well as trends, the most stylish range of shoes & sandals",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 31,
    "currency": "Moroccan Dirham",
    "id": "29"
  },
  {
    "shop": 40,
    "category": [ ],
    "title": "watt",
    "description": "Ergonomic executive chair upholstered in bonded black leather and PVC padded seat and back for all-day comfort and support",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 16,
    "currency": "Yuan Renminbi",
    "id": "30"
  },
  {
    "shop": 93,
    "category": [ ],
    "title": "RAM",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 22,
    "currency": "Rial Omani",
    "id": "31"
  },
  {
    "shop": 73,
    "category": [ ],
    "title": "Dollar",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 63,
    "currency": "Vatu",
    "id": "32"
  },
  {
    "shop": 90,
    "category": [ ],
    "title": "Clovis",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 4,
    "currency": "Lek",
    "id": "33"
  },
  {
    "shop": 53,
    "category": [ ],
    "title": "Books",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 89,
    "currency": "New Taiwan Dollar",
    "id": "34"
  },
  {
    "shop": 49,
    "category": [ ],
    "title": "deposit",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 18,
    "currency": "Fiji Dollar",
    "id": "35"
  },
  {
    "shop": 17,
    "category": [ ],
    "title": "withdrawal",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 67,
    "currency": "Venezuelan bolívar",
    "id": "36"
  },
  {
    "shop": 75,
    "category": [ ],
    "title": "Southeast",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 44,
    "currency": "Hong Kong Dollar",
    "id": "37"
  },
  {
    "shop": 78,
    "category": [ ],
    "title": "bluetooth",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 98,
    "currency": "Kina",
    "id": "38"
  },
  {
    "shop": 63,
    "category": [ ],
    "title": "female",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 94,
    "currency": "Cedi",
    "id": "39"
  },
  {
    "shop": 43,
    "category": [ ],
    "title": "facilis",
    "description": "Andy shoes are designed to keeping in mind durability as well as trends, the most stylish range of shoes & sandals",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 99,
    "currency": "Malaysian Ringgit",
    "id": "40"
  },
  {
    "shop": 82,
    "category": [ ],
    "title": "Clothing",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 59,
    "currency": "Leone",
    "id": "41"
  },
  {
    "shop": 40,
    "category": [ ],
    "title": "Customer",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 26,
    "currency": "UIC-Franc",
    "id": "42"
  },
  {
    "shop": 4,
    "category": [ ],
    "title": "Berkshire",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 75,
    "currency": "Jordanian Dinar",
    "id": "43"
  },
  {
    "shop": 55,
    "category": [ ],
    "title": "Buckinghamshire",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 4,
    "currency": "Latvian Lats",
    "id": "44"
  },
  {
    "shop": 19,
    "category": [ ],
    "title": "North",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 34,
    "currency": "Tenge",
    "id": "45"
  },
  {
    "shop": 21,
    "category": [ ],
    "title": "Hat",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 60,
    "currency": "Afghani",
    "id": "46"
  },
  {
    "shop": 43,
    "category": [ ],
    "title": "bypassing",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 55,
    "currency": "Pound Sterling",
    "id": "47"
  },
  {
    "shop": 84,
    "category": [ ],
    "title": "rubbish",
    "description": "Andy shoes are designed to keeping in mind durability as well as trends, the most stylish range of shoes & sandals",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 53,
    "currency": "Ouguiya",
    "id": "48"
  },
  {
    "shop": 22,
    "category": [ ],
    "title": "Human",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 78,
    "currency": "Quetzal",
    "id": "49"
  },
  {
    "shop": 54,
    "category": [ ],
    "title": "Progressive",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 49,
    "currency": "Belize Dollar",
    "id": "50"
  },
  {
    "shop": 87,
    "category": [ ],
    "title": "up",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 66,
    "currency": "Lesotho Loti",
    "id": "51"
  },
  {
    "shop": 90,
    "category": [ ],
    "title": "preservation",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 97,
    "currency": "Czech Koruna",
    "id": "52"
  },
  {
    "shop": 56,
    "category": [ ],
    "title": "Northwest",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 61,
    "currency": "European Monetary Unit (E.M.U.-6)",
    "id": "53"
  },
  {
    "shop": 48,
    "category": [ ],
    "title": "Sedan",
    "description": "New range of formal shirts are designed keeping you in mind. With fits and styling that will make you stand apart",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 65,
    "currency": "Cordoba Oro",
    "id": "54"
  },
  {
    "shop": 75,
    "category": [ ],
    "title": "Northeast",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 30,
    "currency": "Solomon Islands Dollar",
    "id": "55"
  },
  {
    "shop": 98,
    "category": [ ],
    "title": "program",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 27,
    "currency": "Cayman Islands Dollar",
    "id": "56"
  },
  {
    "shop": 20,
    "category": [ ],
    "title": "JBOD",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 43,
    "currency": "Lithuanian Litas",
    "id": "57"
  },
  {
    "shop": 31,
    "category": [ ],
    "title": "inasmuch",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 17,
    "currency": "Lebanese Pound",
    "id": "58"
  },
  {
    "shop": 96,
    "category": [ ],
    "title": "Home",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 87,
    "currency": "Silver",
    "id": "59"
  },
  {
    "shop": 28,
    "category": [ ],
    "title": "Berkshire",
    "description": "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 70,
    "currency": "Zambian Kwacha",
    "id": "60"
  },
  {
    "shop": 95,
    "category": [ ],
    "title": "Licensed",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 81,
    "currency": "Sri Lanka Rupee",
    "id": "61"
  },
  {
    "shop": 76,
    "category": [ ],
    "title": "Gasoline",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 35,
    "currency": "Russian Ruble",
    "id": "62"
  },
  {
    "shop": 72,
    "category": [ ],
    "title": "quae",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 11,
    "currency": "Zimbabwe Dollar",
    "id": "63"
  },
  {
    "shop": 99,
    "category": [ ],
    "title": "mid",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 68,
    "currency": "Tanzanian Shilling",
    "id": "64"
  },
  {
    "shop": 62,
    "category": [ ],
    "title": "strategic",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 59,
    "currency": "SDR",
    "id": "65"
  },
  {
    "shop": 16,
    "category": [ ],
    "title": "hm",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 48,
    "currency": "Silver",
    "id": "66"
  },
  {
    "shop": 82,
    "category": [ ],
    "title": "Operations",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 74,
    "currency": "Moldovan Leu",
    "id": "67"
  },
  {
    "shop": 11,
    "category": [ ],
    "title": "British",
    "description": "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 71,
    "currency": "Metical",
    "id": "68"
  },
  {
    "shop": 8,
    "category": [ ],
    "title": "ample",
    "description": "The Apollotech B340 is an affordable wireless mouse with reliable connectivity, 12 months battery life and modern design",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 75,
    "currency": "Liberian Dollar",
    "id": "69"
  },
  {
    "shop": 37,
    "category": [ ],
    "title": "chronograph",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 40,
    "currency": "UAE Dirham",
    "id": "70"
  },
  {
    "shop": 97,
    "category": [ ],
    "title": "Van",
    "description": "Andy shoes are designed to keeping in mind durability as well as trends, the most stylish range of shoes & sandals",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 27,
    "currency": "SDR",
    "id": "71"
  },
  {
    "shop": 68,
    "category": [ ],
    "title": "Rustic",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 61,
    "currency": "Gibraltar Pound",
    "id": "72"
  },
  {
    "shop": 46,
    "category": [ ],
    "title": "Awesome",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 31,
    "currency": "Tunisian Dinar",
    "id": "73"
  },
  {
    "shop": 9,
    "category": [ ],
    "title": "Coupe",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 40,
    "currency": "Kenyan Shilling",
    "id": "74"
  },
  {
    "shop": 61,
    "category": [ ],
    "title": "Outdoors",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 51,
    "currency": "Gold",
    "id": "75"
  },
  {
    "shop": 83,
    "category": [ ],
    "title": "boo",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 70,
    "currency": "Falkland Islands Pound",
    "id": "76"
  },
  {
    "shop": 52,
    "category": [ ],
    "title": "overriding",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 14,
    "currency": "Kwanza",
    "id": "77"
  },
  {
    "shop": 82,
    "category": [ ],
    "title": "architecture",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 0,
    "currency": "Chilean Peso",
    "id": "78"
  },
  {
    "shop": 69,
    "category": [ ],
    "title": "Van",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 69,
    "currency": "UIC-Franc",
    "id": "79"
  },
  {
    "shop": 7,
    "category": [ ],
    "title": "Sports",
    "description": "Ergonomic executive chair upholstered in bonded black leather and PVC padded seat and back for all-day comfort and support",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 85,
    "currency": "Armenian Dram",
    "id": "80"
  },
  {
    "shop": 80,
    "category": [ ],
    "title": "Southwest",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 67,
    "currency": "Qatari Rial",
    "id": "81"
  },
  {
    "shop": 67,
    "category": [ ],
    "title": "Cambridgeshire",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 33,
    "currency": "Comoro Franc",
    "id": "82"
  },
  {
    "shop": 76,
    "category": [ ],
    "title": "Intranet",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 29,
    "currency": "Dalasi",
    "id": "83"
  },
  {
    "shop": 32,
    "category": [ ],
    "title": "leverage",
    "description": "Ergonomic executive chair upholstered in bonded black leather and PVC padded seat and back for all-day comfort and support",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 62,
    "currency": "Tala",
    "id": "84"
  },
  {
    "shop": 68,
    "category": [ ],
    "title": "sub",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 25,
    "currency": "Bermudian Dollar (customarily known as Bermuda Dollar)",
    "id": "85"
  },
  {
    "shop": 14,
    "category": [ ],
    "title": "secondary",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 71,
    "currency": "Malaysian Ringgit",
    "id": "86"
  },
  {
    "shop": 94,
    "category": [ ],
    "title": "portals",
    "description": "The slim & simple Maple Gaming Keyboard from Dev Byte comes with a sleek body and 7- Color RGB LED Back-lighting for smart functionality",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 74,
    "currency": "Sri Lanka Rupee",
    "id": "87"
  },
  {
    "shop": 83,
    "category": [ ],
    "title": "Wooden",
    "description": "The automobile layout consists of a front-engine design, with transaxle-type transmissions mounted at the rear of the engine and four wheel drive",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 90,
    "currency": "Solomon Islands Dollar",
    "id": "88"
  },
  {
    "shop": 29,
    "category": [ ],
    "title": "Garden",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 99,
    "currency": "Iranian Rial",
    "id": "89"
  },
  {
    "shop": 85,
    "category": [ ],
    "title": "West",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 42,
    "currency": "Kroon",
    "id": "90"
  },
  {
    "shop": 42,
    "category": [ ],
    "title": "Hyundai",
    "description": "Ergonomic executive chair upholstered in bonded black leather and PVC padded seat and back for all-day comfort and support",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 18,
    "currency": "Belarusian Ruble",
    "id": "91"
  },
  {
    "shop": 44,
    "category": [ ],
    "title": "Games",
    "description": "Boston's most advanced compression wear technology increases muscle oxygenation, stabilizes active muscles",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 46,
    "currency": "US Dollar",
    "id": "92"
  },
  {
    "shop": 50,
    "category": [ ],
    "title": "XSS",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 1,
    "currency": "Dong",
    "id": "93"
  },
  {
    "shop": 36,
    "category": [ ],
    "title": "Manganese",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 72,
    "currency": "Fiji Dollar",
    "id": "94"
  },
  {
    "shop": 79,
    "category": [ ],
    "title": "unusual",
    "description": "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 72,
    "currency": "Baht",
    "id": "95"
  },
  {
    "shop": 72,
    "category": [ ],
    "title": "ding",
    "description": "Andy shoes are designed to keeping in mind durability as well as trends, the most stylish range of shoes & sandals",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 88,
    "currency": "Kyat",
    "id": "96"
  },
  {
    "shop": 75,
    "category": [ ],
    "title": "blissfully",
    "description": "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 19,
    "currency": "Pataca",
    "id": "97"
  },
  {
    "shop": 79,
    "category": [ ],
    "title": "UTF8",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 8,
    "currency": "Armenian Dram",
    "id": "98"
  },
  {
    "shop": 14,
    "category": [ ],
    "title": "Hybrid",
    "description": "New ABC 13 9370, 13.3, 5th Gen CoreA5-8250U, 8GB RAM, 256GB SSD, power UHD Graphics, OS 10 Home, OS Office A & J 2016",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 9,
    "currency": "Uzbekistan Sum",
    "id": "99"
  },
  {
    "shop": 91,
    "category": [ ],
    "title": "portals",
    "description": "The Nagasaki Lander is the trademarked name of several series of Nagasaki sport bikes, that started with the 1984 ABC800J",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 92,
    "currency": "Forint",
    "id": "100"
  },
  {
    "shop": 42,
    "category": [ ],
    "title": "repellat",
    "description": "The beautiful range of Apple Naturalé that has an exciting mix of natural ingredients. With the Goodness of 100% Natural Ingredients",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 72,
    "currency": "Liberian Dollar",
    "id": "101"
  },
  {
    "shop": 1,
    "category": [ ],
    "title": "woman",
    "description": "The Football Is Good For Training And Recreational Purposes",
    "image": "https://loremflickr.com/640/480/cats",
    "price": 39,
    "currency": "European Unit of Account 9(E.U.A.-9)",
    "id": "102"
  }
]


let cartItems = [];
const container = document.getElementById('container');
const searchValue = document.getElementById('search');
const searchButton = document.getElementById('searchBTN');
const min = document.getElementById('priceInput1');
const max = document.getElementById('priceInput2');
const cart = document.getElementById('cart');

function deleteFronCart(index) {
  cartItems.splice(index, 1);
  renderCartItems(cartItems);
}

function renderItems(array) {
  container.innerHTML = '';
  array.forEach((element, index) => {
    container.innerHTML += `
      <div class="card">
        <img src="${element.image}" alt="">
        <h1>${element.title}</h1>
        <p>${element.description}</p>
        <div>
          <h2>${element.price}</h2>
          <b>${element.currency}</b>
        </div>
        <button onclick="addTocart(${index})" id="btn">Add to cart</button>
        <button class="modalButton" onclick="modalWindow(${index})">Learn more</button>
      </div>
    `;
  });
  
}

function addTocart(index) {
  cartItems.push(data[index]);
  renderCartItems(cartItems);
}

function renderCartItems(array) {
  cart.innerHTML = '';
  array.forEach((element, index) => {
    cart.innerHTML += `
      <div class="cartItem">
        <img width="30px" class="cartImage" src="${element.image}" alt="">
        <div>
          <h1>${element.title}</h1>
          <h2>${element.price} <u>${element.currency}</u></h2>
        </div>
        <button onclick="deleteFronCart(${index})" class="deleteFromCart">🗑️</button>
      </div>
    `;
  });
}

function searchItems() {
  const filteredData = data.filter(el => el.title.toLowerCase().includes(searchValue.value.toLowerCase()));
  renderItems(filteredData);
}
function clearFilter() {
  renderItems(data);
}

function sortByPrice() {
  const minPrice = +min.value;
  const maxPrice = +max.value;
  

  const filteredProducts = data.filter(el => el.price >= minPrice && el.price <= maxPrice);
  const sortedProducts = filteredProducts.sort((a, b) => a.price - b.price);

  renderItems(sortedProducts);
}

function modalWindow(index) {
  const window = document.getElementById("modal");
  const element = data[index];

  if (element && element.description) {
    const modal = document.createElement("div");
    modal.id = "modalWindow";
    modal.className = "modalWindow";
    modal.innerHTML = `
    <img src="${element.image}" alt="" class="modalImg">
    <div class="titleDes">
    <h1>${element.title}</h1>
    <p>${element.description}</p>
    </div>
    <div class="priceCurrency">
    <h3>${element.price}</h3>
    <h3>${element.currency}</h3>
    </div>
    
    `;

    const close = document.createElement("button");
    close.className = "closeButton";
    close.innerText = "Close";
    close.addEventListener("click", () => {
      modal.remove();
    });

    modal.appendChild(close);
    window.appendChild(modal);
  } else {
    console.error("Invalid element or missing description property");
  }
}


renderItems(data);